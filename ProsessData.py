...
def process_and_export_schedule(table, save_path, save_data):
    ...
    day_order = ['Tuesday', 'Wednesday', 'Thursday',
    ...
    output_df = output_df.sort_values(
        by=['Day', 'Shift', 'Experience level num']).reset_index(drop=True)

    # Step 2: Initialize 'Notes' column
    output_df['Notes'] = ''

    # 2. Shift Name == 'Afternoon' → 'peak coverage' (don't overwrite existing notes)
    output_df.loc[(output_df['Shift'] == 'Afternoon') & (
        output_df['Notes'] == ''), 'Notes'] = 'peak coverage'
    ...
    final_columns = ['Day', 'Name', 'Shift', '1st Preferred shift',
                     'Experience level', 'Total Shifts', 'Shift Night', 'Weekend', 'Notes']
