...
from writeHere import columns_name,Percentage_all
...
def calculate_numEmp_req_experience_shift(df, exp_level=5):
...
def get_experience_shift_counts(df, shift_col='1st Preferred shift', exp_col='Experience level num'):
    ...
    pivot_df = df.groupby([exp_col, shift_col]).size().unstack(
...
def get_count_and_index(df, shift, exp_level):
    ...
    for i, row in df.iterrows():
        ...
        # Skip the first column (exp level)
        for current_shift in df.columns[1:]:
...
def employee_schedule(df, req_emp_in_shift, count_emp_in_exp, index, name_shift, day, table):
    ...
    if count_emp_in_exp > req_emp_in_shift:
        ...
        shape_data = df[index + req_emp_in_shift:index +
                        count_emp_in_exp].shape[0]

        insert_emp_in_table(data, name_shift, day, table)

        if shape_data == 1:
            df.at[df.index[index + req_emp_in_shift], 'Shift Night'] += 1
            data_dis = df[index + req_emp_in_shift:index +
                          count_emp_in_exp].values.tolist()
            insert_emp_in_table(data_dis, shift='Overnight',
                                day=day, table=table)
        else:
            i = (index+req_emp_in_shift)+(count_emp_in_exp-req_emp_in_shift)//2
            i_i = (index+req_emp_in_shift) + \
                ((count_emp_in_exp-req_emp_in_shift)//2)+1
            j = index+count_emp_in_exp
            if shape_data % 2 == 0:
                df.loc[i:j, ['Shift Night']] += 1
            else:
                df.loc[i_i:j, ['Shift Night']] += 1
            data = split_into_equal_parts(
                df[index+req_emp_in_shift:index+count_emp_in_exp].values.tolist(), 2)
            insert_emp_in_table(
                list(data[0]), shift='Afternoon', day=day, table=table)
            insert_emp_in_table(
                list(data[1]), shift='Overnight', day=day, table=table)
    else:
        insert_emp_in_table(
            data=df[index:index + count_emp_in_exp].values.tolist(), shift=name_shift, day=day, table=table)
    ...
    for num_shift, shift in enumerate(Shifts):
        ...
        if num_shift == 0:
            for exp_level, req_in_shift in enumerate(num_of_employees_per_shift_experience[shift]):
                ...
                cnt, index = get_count_and_index(
                    experience_counts, shift=shift, exp_level=exp_level + 1)

                employee_schedule(df, req_in_shift, cnt,
                                  index, shift, day, table)
